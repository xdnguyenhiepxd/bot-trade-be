export function randomUserAgent(isMobile = false) {
  const browserStringsMobile = [
  ...
  if (isMobile) {
    return browserStringsMobile[Math.floor(Math.random() * browserStringsMobile.length)]
  }
  const browserFreq = {
  ...
  let rcount = 0
  let browserType = ""
  for (const k in browserFreq) {
    max += browserFreq[k]
  }
  const roll = Math.floor(Math.random() * max)
  for (const k in browserFreq) {
    rcount += browserFreq[k]
    if (roll < rcount && browserType === "") {
      browserType = k
      break
    }
  }
  const userAgentArray = browserStrings[browserType]
